...
namespace Gibe.Umbraco.IntelligentMedia.Azure
{
	public class AzureVisionResponse : IVisionResponse
	{
		public AzureVisionResponse(AzureVisionApi.VisionResponse response, string json)
		{
			...
			Description = response.Description.Captions.OrderByDescending(c => c.Confidence).Select(t => new ProbableTag { Confidence = t.Confidence, Tag = t.Text }).ToList();
			if (response.Categories != null)
			{
				Categories = response.Categories.Select(t => new ProbableTag { Confidence = t.Score, Tag = t.Name.Replace("_", " ") }).ToList();
			}
