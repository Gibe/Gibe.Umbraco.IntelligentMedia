...
namespace Gibe.Umbraco.IntelligentMedia.Google
{
	public class GoogleVisionApi : IVisionApi
	{
		...
		public GoogleVisionApi(IntelligentMediaSettings settings)
		{
		...
		public async Task<IVisionResponse> MakeRequest(IMedia media)
		{
			...
			var umbracoFile = JsonConvert.DeserializeObject<AzureVisionApi.UmbracoFileData>(umbracoFileString);
			var imageData = Convert.ToBase64String(GetImageAsByteArray(HttpContext.Current.Server.MapPath(umbracoFile.Src)));
